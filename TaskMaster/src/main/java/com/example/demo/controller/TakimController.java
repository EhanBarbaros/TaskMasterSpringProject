...
/*
    ...
    public ResponseEntity<?> createTeam(@RequestParam("teamName") String teamName, @RequestParam("userId") Long userId) {
        ...
        newTakim.setTakimLideriKullaniciId(user.getId());
        newTakim.setUyeSayisi(1);  

        TakimEntity createdTakim = takimService.createTakim(newTakim);
        user.setTakimId(createdTakim.getTakimId()); 
        usersService.saveOrUpdateUser(user);

        return ResponseEntity.ok(createdTakim);
    }
    */
    @PostMapping("/createTeam")
